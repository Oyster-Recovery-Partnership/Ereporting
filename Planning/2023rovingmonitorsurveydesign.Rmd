...
RM_FH = dplyr::select(RM_FH, TripID, type, Date, MonitorReport,TimeChecked)
...
*  Shellfish:
     ...
     +  Razor Clams are open year round except Sunday
...
*  Charter: see full species opening dates in list in the [confluence link](https://oysterrecoverypartnership.atlassian.net/wiki/spaces/EREPORTING/pages/1819213825/Fisheries+Dates)
     +  Striped Bass are open for trophy season in early May, then open 6/1 - 12/31
...
ggplot() + 
  geom_polygon(data = zips_df, aes(y = lat, x = long,  fill = n, group = group)) + 
  labs(x = "Landing Hour", y = "Number of Trips", fill = "Fishery", title = "Number of Trips in Each Zip Code") + 
  theme_bw() +
  theme(text = element_text(size = 15),
        legend.position = "bottom") 
```
