...
RM_SF = dplyr::select(RM_SF, TripID, type, Date, MonitorReport,TimeChecked)
RM_FH = dplyr::select(RM_SF, TripID, type, Date, MonitorReport,TimeChecked)
RM_FF_BC = dplyr::select(RM_FF_BC, TripID, type, Date, MonitorReport, Time) %>%
...
- Charter: see full species opening dates in list in link above  
-- Striped Bass
- Finfish: see full species open dates in list in link above
-- Yellow Perch
-- Striped Bass
- Blue Crab  
-- Dates differ for hard, soft, and peelers but generally encompassed in 4/1 - 12/15
...
ggplot() + 
  geom_bar(data = trip_sum_byDay, aes(x=d, y=n, fill = type), 
           stat = "identity" , position = "stack") + 
  labs(x = "Day of the Week", y = "Number of Trips", fill = "Fishery") + 
  theme_bw() +
  theme(text = element_text(size = 20)) 
```
...
- For finfish and bluecrab we had  
-- 
...
ggplot() + 
  geom_bar(data = trip_sum_byHour, aes(hr, n, fill = type), 
           stat = "identity" , position = "stack") + 
  labs(x = "Langing Hour", y = "Number of Trips", fill = "Fishery") + 
  theme_bw() +
  theme(text = element_text(size = 20)) 
# tables
```
...
Data are summarize by zip code to help inform where to divide the regions for effort. Currently the plan is to have six roving monitors, so 6 regions. There will additionally be two onboard observers. For the onboard observers the regions would be divided by the bay bridge so an analysis for this division is not needed. 
```{r}
...
```{r}
# plot for shellfish gears
```


## Past reports
Please see this past reports for more guideance. 
